<template>
    <div class="flex flex-col items-center justify-center min-h-screen gap-4">
        <form  @submit.prevent="handleSubmit" class="w-full max-w-xs space-y-4">
            ...
            <input  v-model="email" required type="email" placeholder="Email" class="input input-md input-bordered input-info w-full block" />
            <input  v-model="password" required type="password" placeholder="Password" class="input input-md input-bordered input-info w-full block" />
            <button  @click="submit" class="btn w-full">Masuk</button>
  ...
  methods: {
    async handleSubmit() {
      ...
      if (response.status == "200") {
        ...
        this.$router.push(`/yourcerpen/${response.data.data.id}/cerpen`);
      } else {
